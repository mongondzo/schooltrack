# Feature Schedules — SchoolTrack
## Guide d'intégration complet (pour débutants)

---

## 📁 Arborescence complète générée

```
features/schedules/
│
├── domain/                               ← Logique pure, sans Firestore
│   ├── entities/
│   │   └── schedule_entity.dart          ← L'objet ScheduleEntity
│   └── repositories/
│       └── schedule_repository.dart      ← Contrat abstrait (interface)
│
├── data/                                 ← Tout ce qui touche Firestore
│   ├── models/
│   │   └── schedule_model.dart           ← Conversion JSON ↔ Dart
│   ├── datasources/
│   │   └── schedule_remote_datasource.dart  ← Appels Firestore directs
│   └── repositories/
│       └── schedule_repository_impl.dart    ← Implémentation du contrat
│
└── presentation/                         ← Interface utilisateur
    ├── bloc/
    │   ├── schedule_event.dart           ← Actions (LoadSchedules…)
    │   ├── schedule_state.dart           ← États (Loading, Loaded…)
    │   └── schedule_bloc.dart            ← Cerveau de la feature
    ├── pages/
    │   ├── schedules_page.dart           ← Liste par jour + filtre classe
    │   ├── add_schedule_page.dart        ← Formulaire d'ajout
    │   ├── edit_schedule_page.dart       ← Formulaire de modification
    │   └── schedule_details_page.dart    ← Détails d'un créneau
    ├── widgets/
    │   ├── app_colors.dart               ← Couleurs centralisées
    │   ├── schedule_card.dart            ← Carte d'un créneau
    │   ├── schedule_form.dart            ← Formulaire réutilisable
    │   ├── empty_schedules_widget.dart   ← État "liste vide"
    │   └── schedule_stat_card.dart       ← Carte dashboard
    └── routes/
        └── schedule_routes.dart          ← Routes nommées
```
...
Ces packages doivent déjà être présents (utilisés par les autres features) :
...
dependencies:
  ...
  intl: ^0.19.0          # Pour formater la date dans ScheduleDetailsPage
```

Si `intl` manque, ajoutez-le puis lancez :
```bash
flutter pub get
```
